...
/* True while current is flowing INTO the cells - a wall charger in
 * Charge state, or regen braking in Drive. The datasheet gives
 * different (tighter) limits for this direction than for discharge,
 * and the cells don't care which state the top-level FSM is in -
 * only which way the current is actually flowing right now. */
static int is_charging(const bms_inputs_t *in) {
    return in->pack_current_a < 0.0f;
}

static uint32_t evaluate_faults(const bms_inputs_t *in) {
    uint32_t flags = FAULT_NONE;
    float temp_fault_limit = is_charging(in) ? TEMP_FAULT_CHARGE_C : TEMP_FAULT_DISCHARGE_C;
    ...
    for (int i = 0; i < NUM_MODULES; i++) {
        ...
        if (in->cell_temp_c[i]  > temp_fault_limit)  flags |= FAULT_OVERTEMP;
    }
...
static int needs_derate(const bms_inputs_t *in) {
    float temp_warn_limit = is_charging(in) ? TEMP_WARN_CHARGE_C : TEMP_WARN_DISCHARGE_C;
    for (int i = 0; i < NUM_MODULES; i++) {
        if (in->cell_temp_c[i] > temp_warn_limit)         return 1;
        if (in->cell_voltage[i] < CELL_V_UNDERVOLT_WARN)  return 1;
    }
    if (in->pack_current_a > CURRENT_WARN_A)        return 1;
    if (in->pack_current_a < REGEN_CURRENT_WARN_A)  return 1;
    return 0;
}
