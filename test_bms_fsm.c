...
TEST(undervoltage_faults_during_drive) {
    ...
    in.driver_hv_request = 1;
    in.bus_voltage = pack_v();
    in.cell_voltage[5] = 2.4f; /* under CELL_V_UNDERVOLT (2.50) */
    bms_event_t ev = bms_fsm_step(&fsm, &in);
...
TEST(warm_cell_derates_without_faulting) {
    ...
    in.driver_hv_request = 1;
    in.bus_voltage = pack_v();
    in.cell_temp_c[7] = 55.0f; /* over TEMP_WARN_DISCHARGE_C (50), under TEMP_FAULT_DISCHARGE_C (60) */
    bms_event_t ev = bms_fsm_step(&fsm, &in);
...
TEST(regen_overcurrent_faults) {
    ...
    in.driver_hv_request = 1;
    in.bus_voltage = pack_v();
    in.pack_current_a = -25.0f; /* past REGEN_CURRENT_FAULT_A (-20) */
    bms_event_t ev = bms_fsm_step(&fsm, &in);
...
TEST(mild_regen_is_fine) {
    ...
    in.driver_hv_request = 1;
    in.bus_voltage = pack_v();
    in.pack_current_a = -8.0f; /* under REGEN_CURRENT_WARN_A (-12), truly mild */
    bms_event_t ev = bms_fsm_step(&fsm, &in);
